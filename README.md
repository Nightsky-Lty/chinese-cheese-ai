...
> 置换表，以及“三次重复、长将、无吃子”的简化裁决均已实现；**长捉判定尚未
> 实现**，神经网络、训练流水线和 GUI 自动操作也尚未实现。
...
**当前没有实现长捉逻辑。** 裁决器不会识别被追逐棋子、捉子是否连续、交换价值
或“一将一捉”等复杂责任。因此，非单方长将造成的三次重复目前一律判和。
...
2. 设计并实现长捉识别，记录受威胁棋子并分析重复区间内双方的循环责任。
3. 完善着法排序和搜索剪枝，并加入时间管理，形成稳定的传统引擎基线。
4. 用 Python 建立自我对弈、棋谱解析、样本生成、训练和模型评估流程。
5. 在 C++ 中实现轻量神经网络推理，用训练模型替换或增强手工评估。
6. 定义稳定的引擎通信接口，再单独实现棋盘识别与 GUI 操作模块。
